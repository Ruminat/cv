  ...
  return (
    <Reveal className="mb-8 flex items-center justify-between gap-4">
      <h2 className="flex items-center gap-3 text-2xl font-bold tracking-tight md:text-3xl">
        <span className="flex size-9 items-center justify-center rounded-xl border border-brand-orange/25 bg-brand-orange/10 text-brand-orange">
          <Icon className="size-5" />
        </span>
        {title}
      </h2>
      {meta ? (
        <span className="text-sm font-medium text-brand-purple">{meta}</span>
      ) : null}
    </Reveal>
  )
}
