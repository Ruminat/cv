...
test.describe("main page", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/");
    await expect(
      page.getByRole("heading", { name: "Vlad Furman", level: 1 }),
    ).toBeVisible();
  });

  test("renders the hero identity and key stats", async ({ page }) => {
    await expect(
      page.getByText("Senior Frontend Engineer").first(),
    ).toBeVisible();
    await expect(
      page.getByText("Open to relocation to UAE / Europe"),
    ).toBeVisible();
  ...
  test('opens and closes the "Let\'s talk" contact dialog', async ({
    page,
  }) => {
